export const getZip = async (location = '') => {
  ...
  try {
    const zipcodeResponse = await fetch(
      `https://www.zipcodeapi.com/rest/${apiKeyClientSide}/city-zips.json/${locationArray[0].trim()}/${locationArray[1].trim()}`
    );
    const zipcodeResponseJson = await zipcodeResponse.json();
    const longLatResponse = await fetch(
      `https://www.zipcodeapi.com/rest/${apiKeyClientSide}/info.json/${zipcodeResponseJson.zip_codes[0]}/degrees`
    );
